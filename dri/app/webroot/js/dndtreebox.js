...
treeJSON = d3.json("nodos/origen/1", function(error, treeData) {

	root = treeData;
	var tree = d3.layout.tree().nodeSize([125, 40]);
	var diagonal = d3.svg.diagonal().projection(function(d) {
		return [d.x + rectW / 2, -d.y + rectH / 2];
		//return [d.x, d.y];
	});

	var svg = d3.select("#tree-container").append("svg").attr("width", 1000).attr("height", 1000).call( zm = d3.behavior.zoom().scaleExtent([0.1, 3]).on("zoom", redraw)).append("g").attr("transform", "translate(" + 350 + "," + 420 + ")");

	//necessary so that zoom knows where to zoom and unzoom from
	zm.translate([350, 420]);
